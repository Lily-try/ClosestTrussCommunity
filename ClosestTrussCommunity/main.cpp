...
double CTC() {
 ...
 // 把边加进去
    ...
    cerr << "Overall running time: " << double(endend - start) / CLOCKS_PER_SEC << "s. ";
    cerr << "CTC completed." << endl;
    return double(endend-start)/CLOCKS_PER_SEC; //返回耗时（秒）
    // assert(0);
}
...
//    freopen(output_name.c_str(), "a", stdout);
     string output_name = "./output/" + dataset + "_q";
//    output_name = "./output/"+dataset+"_summary.txt";
    output_name = "./Dataset/"+dataset+"/output.txt"; //lwl
    cerr<<"结果存入了"<<output_name<<endl;
    freopen(output_name.c_str(), "a", stdout);
...
//    cout<< tq <<" ";
    ...
    cout << endl;
    cout << fixed << setprecision(6) <<query_time<< endl; //存入查询时间
    // for(auto u : ans.V) {
    //     for(auto v : ans.G[u]) {
    //         if(u < v) cout << u << " " << v << endl;
    //     }
