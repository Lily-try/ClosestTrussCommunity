...
def CommunitySearch(args):
    ...
    val_bst_f1_model = copy.deepcopy(model.state_dict())  # 存储最优的模型
    val_bst_pre_model = copy.deepcopy(model.state_dict())  # 存储最优的模型
    ...
    for epoch in range(args.epoch_n): #训练模型
        ...
        with torch.no_grad():
            ...
            s_, f1_ = validation__(val,cluster_membership,sg_nodes,n_nodes, nodes_feats,device,g, model,nodes_adj,args.k)
            sp_, pre_ = validation_part_pre(val,cluster_membership,sg_nodes,n_nodes, nodes_feats,device,g, model,nodes_adj,args.k)
            val_time = (datetime.datetime.now() - val_start).seconds  # 这两个验证的时间
            val_epochs_time = val_epochs_time + val_time
        if f1_ > val_best_f1:
            val_best_f1 = f1_
            val_bst_f1_ep = epoch
            val_bst_f1_model = copy.deepcopy(model.state_dict())  # 拷贝的是最佳的权重
            logger.info(f"Type of val_best_model: {type(val_bst_f1_model)}")
            val_bst_f1_time = (
                                          datetime.datetime.now() - train_start).seconds - val_epochs_time  # 当前这个epoch的时间减去训练后的时间,将验证的时间减去？？
        if pre_ > val_bst_pre:
            val_bst_pre = pre_
            val_bst_pre_ep = epoch
            val_bst_pre_model = copy.deepcopy(model.state_dict())  # 拷贝的是最佳的权重
    ...
    logger.info(
        f'===best Pre at epoch {val_bst_pre_ep}, Best Precision:{val_bst_pre} ===,Best epoch time:{val_bst_pre_time}')
    ...
    with th.no_grad():
        ...
        if args.val_type == 'f1':
            ...
            s_, f1_ = validation_part_pre(val, cluster_membership, sg_nodes,
                                   n_nodes, nodes_feats, device, g, model, nodes_adj,args.k)
    ...
    with open(output, 'a+',encoding='utf-8') as fh:
        ...
        line = (
            ...
            f"jac_score: {jac_score}\n"
             f"f1_: {f1_score}, pre_:{precision},rec_:{recall}\n"
