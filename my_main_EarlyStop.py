...
from utils.val_utils import f1_score_, NMI_score, ARI_score, JAC_score, get_res_path, get_model_path
...
def Community_Search(args,logger):
    ...
    val_best_res = 0. #记录最优的结果
    val_best_ep = 0 #记录取得最优结果的epoch
    val_best_model = copy.deepcopy(embLearner.state_dict()) #存储最优的模型
    #模型训练阶段
    ...
    for epoch in range(args.epoch_n):
        ...
        if f1_ > val_best_res:
            val_best_res = f1_
            val_best_ep = epoch
            val_best_model = copy.deepcopy(embLearner.state_dict()) #拷贝的是最佳的权重
            logger.info(f"Type of val_best_model: {type(val_best_model)}")
        else:
            if epoch - val_best_ep > args.patience:
                logger.info(f'=== Early Stop at epoch {val_best_ep + 1}, Best F1:{val_best_res} ===')
                break #跳出循环，结束训练
        check_time = (datetime.datetime.now() - check_start).seconds
        logger.info(f'To Early Stopping Validation F1:{f1_}, Time:{check_time}s')
    #运行完所有epoch的时间
    training_time = (datetime.datetime.now() - train_start).seconds-check_time #不额外记录early_stop所耗费的时间
    ...
    torch.save(val_best_model, model_path)  # torch.save(embLearner.state_dict(), model_path)
    ...
    with open(output, 'a+',encoding='utf-8') as fh:
        ...
        line = (
            f"args: {args}\n"
            f"bst_epoch:{val_best_ep}, bst_ep_f1:{val_best_res}\n"
