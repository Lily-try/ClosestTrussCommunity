...
void CPM::runBaseline(string dir)
{
	...
	for (int i = 0; i < queryNodeSet.size(); ++i)
	{
		...
		for(int j = 0; j < queryNodeSet[i].size(); ++j)
		{
			...
			char st[16];
//			sprintf( st, "%d",j+1); //格式化字符串，将整数j+1转换为字符串形式，存储在st中xqxq
//			CPM::output_name = dir + "clique/output_clique_" + string(st) + ".txt";
            sprintf(st, "%d", *(queryNodeSet[i][j].begin()));
            CPM::output_name = dir + "clique/output_clique_" +string(st)+ ".txt";
