...
class EmbLearnerKNNGrad(nn.Module):
    ...
    def hyperedge_representation(self, x, edge_index):
        ...
        return hyperedge
    def compute_loss(self,train,edge_weights):
        '''
        ...
        '***********  第0层************************'
        # normal encoder
        hq = F.relu(self.query_layers[0](querys, edge_index,edge_weights)).to(self.device)  # hq=115*256 query_encoder[0]
        h = F.relu(self.layers[0](feats, edge_index,edge_weights)).to(self.device)  # h=115*256  graph_encoder[0]
        ...
        hf = F.relu(hf + self.last_fusion_layers[0](hf_, edge_index,edge_weights))  # fusion_encoder[0]
        ...
        for _ in range(self.num_layers - 2):
            ...
            #进入l+1层
            hq = F.relu(self.query_layers[_ + 1](hq, edge_index,edge_weights)) # query_encoder[_+1]
            h = F.relu(self.layers[_ + 1](h, edge_index,edge_weights)) #graph_encoder[_+1]
            ...
            hf = F.relu(hfx + self.last_fusion_layers[_ + 1](hf, edge_index,edge_weights))
        ...
        '*********最后一层******'
        hq = self.query_layers[self.num_layers - 1](hq, edge_index,edge_weights)
        h = self.layers[self.num_layers - 1](h, edge_index,edge_weights)
        ...
        hf = hfx + self.last_fusion_layers[self.num_layers - 1](hf, edge_index,edge_weights) #（115，256）
    ...
    def forward(self, train,edge_weights):
        if self.training ==False:
            h_ = self.valiates(train)
            return h_
        #否则计算对比损失
        loss,h_ = self.compute_loss(train,edge_weights)
