...
class EmbLearnerwithWeights(nn.Module):
    ...
    def compute_loss(self,train,edge_weight=None):
        '''
        ...
        '***********  第0层************************'
        # normal encoder
        hq = F.relu(self.query_layers[0](querys, edge_index,edge_weight)).to(self.device)  # hq=115*256 query_encoder[0]
        h = F.relu(self.layers[0](feats, edge_index,edge_weight)).to(self.device)  # h=115*256  graph_encoder[0]
        ...
        if self.use_hypergraph:
            h_augq = F.relu(self.aug_query_layers[0](querys, edge_index_aug))  # querys(115,1)---GCNConV(1,256)-->h_augq(115,256)
        ...
        hf = F.relu(hf + self.last_fusion_layers[0](hf_, edge_index,edge_weight))  # fusion_encoder[0]
        ...
        for _ in range(self.num_layers - 2):
            ...
            #进入l+1层
            hq = F.relu(self.query_layers[_ + 1](hq, edge_index,edge_weight)) # query_encoder[_+1]
            h = F.relu(self.layers[_ + 1](h, edge_index,edge_weight)) #graph_encoder[_+1]
            ...
            hf = F.relu(hfx + self.last_fusion_layers[_ + 1](hf, edge_index,edge_weight))
        ...
        #最后一层不使用激活函数
        hf = hfx + self.last_fusion_layers[self.num_layers - 1](hf, edge_index,edge_weight) #（115，256）\
    ...
    def valiates(self,train,edge_weight=None):
        ...
        hf = F.relu(hf + self.last_fusion_layers[0](hf_, edge_index,edge_weight))
        ...
        hfx = torch.sum(hfx, dim=1)
        #最后一层不使用激活函数
        hf = hfx + self.last_fusion_layers[self.num_layers - 1](hf, edge_index,edge_weight)
...
class MlpEdgeWeightPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(MlpEdgeWeightPredictor, self).__init__()
